 ...
 * @package d3yii2\d3printeripp\controllers
 */
class DeviceInfoController extends LayoutController
{
     ...
     */
    public $menuRoute = 'd3printeripp/device-info/index';
    /**
     * @var D3Printer
     */
    ...
    public function actionSetDefaultSettings(string $component): Response
    {
        try {
            ...
            return $this->redirect(['/d3printeripp/device-info', 'component' => $component]);

        } catch (Exception $e) {
            FlashHelper::addDanger('Errors occured: ' . $e->getMessage());
            Yii::error($e->getMessage(), 'd3printeripp-error');
            Yii::error($e->getTraceAsString(), 'd3printeripp-error');
